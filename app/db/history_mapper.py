...
from app.models.history import PhotoStyleHistory  # 引入历史模型
...
def update_history_feedback(history_id: int, makeup_rating: int, outfit_rating: int, pose_rating: int, feedback_comment: str | None) -> dict:
    """更新历史记录反馈并返回最新记录"""
    ...
    try:  # 开始事务
        ...
        result = history.to_dict()  # 转换为字典
        result["avg_score"] = round((makeup_rating + outfit_rating + pose_rating) / 3, 2)  # 计算平均分
        return result  # 返回最新字典
    finally:  # 无论如何都关闭会话
        db.close()  # 关闭数据库会话


def get_history_record(history_id: int) -> dict:
    """根据ID获取历史记录"""
    db = SessionLocal()  # 创建数据库会话
    try:  # 开始查询
        history = db.query(PhotoStyleHistory).filter(PhotoStyleHistory.id == history_id).first()  # 查找历史记录
        ...
        result = history.to_dict()  # 转换为字典
        result["avg_score"] = round((history.makeup_rating + history.outfit_rating + history.pose_rating) / 3, 2)  # 计算平均分
        return result  # 返回结果
    finally:  # 无论如何都关闭会话
        db.close()  # 关闭数据库会话
